...
#include "Wheel.h"
#include <unistd.h> 
#include <boost/bind.hpp>   // 主要的头文件，用于创建绑定函数
...
public:
	...
	bool isMove = false;
	int justSend = 0;//刚刚发送的  stop =0；move =1； turn =2
	int justSpeed ;
	int  justTurn;
	int speed = 0;

void twistCallback(const geometry_msgs::Twist::ConstPtr& msg,Wheel& wheel,MoveIt_Control& moveit_server) {

    isReceiveMsg = true;
	long long  speed = (long long )(msg->linear.x * 300.0);
	std::cout<<std::dec<<speed<<endl;
	int turn = msg->angular.z *100;
	
	if(!isMove || speed != justSpeed ) {
		if(msg->linear.x>0 ){	
			wheel.move(speed,0);
			isMove = true;
			justSend = 1;
	...
	if(msg->linear.z>0){
		...
		joints ={7.06/180*3.1415,63.741/180*3.1415,-83.1/180*3.1415,109.3/180*3.1415,93.9/180*3.1415,99.0/180*3.1415};
		moveit_server.move_j(joints);
	}
	if(msg->linear.z<0){
		vector<double> joints ;
		joints ={92.3/180*3.1415,74.7/180*3.1415,-81.7/180*3.1415,109.3/180*3.1415,89.4/180*3.1415,99.1/180*3.1415};
		moveit_server.move_j(joints);
	}

    std::cout<<msg->linear.x<<msg->linear.y<< msg->linear.z<<msg->angular.x<<msg->angular.y<<msg->angular.z<<std::endl;
}
...
int main(int argc, char** argv) {
	...
	Wheel wheel(nh);
	wheel.init(config);
	if(wheel.receiveAndPublish()==1){
	    cout<<"success THREAD"<<endl;
		}else{
		cout<<"erro THREAD"<<endl;

	}

	int acclerationRatio = 1;
	wheel.setAcceleration(acclerationRatio);
	wheel.getPDO_vel();

    ros::Subscriber twist_subscriber = nh.subscribe<geometry_msgs::Twist>("cmd_vel",1 , boost::bind(&twistCallback, _1, boost::ref(wheel), boost::ref(moveit_server)));
    ros::Rate rate(2); 
    while(ros::ok()){
        if(!isReceiveMsg && isMove){
			if(justSend!=0){
				wheel.stopWithVel0();
				isMove =false;
				justSend = 0;
