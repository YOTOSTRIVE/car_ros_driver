...
void execute_callback(const control_msgs::FollowJointTrajectoryGoalConstPtr& goalPtr, Server* moveit_server)
{
    ...
    std_msgs::Float32MultiArray trajectory_data_msg;
    for (int i = 0; i < n_tra_Points; i++)
    {
        std::vector<double> joint_positions;
        for (int j = 0; j < n_joints; j++)
        {
            joint_positions.push_back(goalPtr->trajectory.points[i].positions[j]);
        }
        trajectory_data_msg.data.insert(trajectory_data_msg.data.end(), joint_positions.begin(), joint_positions.end());
    }
    trajectory_pub.publish(trajectory_data_msg);
...
int main(int argc, char *argv[])
{
    ...
    Server moveit_server(nh,"arm_controller/follow_joint_trajectory", boost::bind(&execute_callback, _1, &moveit_server), false);
    // 手动启动服务器
     trajectory_pub = nh.advertise<std_msgs::Float32MultiArray>("/trajectory/points", 10);
